#!/usr/bin/env python
# $Id: urllib2_file.py,v 1.5 2009-06-07 09:26:50 kosh Exp $
...
def send_data(v_vars, v_files, boundary, sock=None):
    ...
    for (k, v) in v_files:
        fd = v
        fd.seek(0,2) #Seek to the end of the file
        file_size = file_size = fd.tell()
        fd.seek(0)
        if hasattr(fd, 'name'):
            name = fd.name.split('/')[-1]
        else:
            name = repr(time.time()).replace('.', '_')
        buffer=''
        buffer += '--'+boundary+'\r\n'
...
class newHTTPHandler(urllib2.BaseHandler):
    ...
    def do_open(self, http_class, req):
        ...
        if req.has_data() and type(data) != str:
            ...
            for (k, v) in data:
                if hasattr(v,'seek') and hasattr(v, 'read') and hasattr(v, 'tell'):
                    v_files.append((k, v))
                else:
                    v_vars.append( (k, v) )
...
urllib2.HTTPSHandler = newHTTPHandler
